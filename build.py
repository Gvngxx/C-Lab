...
def build():
    ...
    # 1. GESTION DE GLFW
    glfw_third_party_path = os.path.join(BASE_DIR, "third_party", "glfw", "build", "src", "third_partyglfw3.a")
    
    if not os.path.exists(glfw_third_party_path):
        build_dir = os.path.join(BASE_DIR, "third_party", "glfw", "build")
        ...
        ejecutar_con_animacion(f'"{MAKE}"', "[INFO] Compilando third_partyrerias GLFW", "Fallo en Make.", cwd=build_dir)
    else:
    ...
    sources = [
        r"src\core\main.cpp", r"src\graphics\ShaderProgram.cpp", r"src\input\InputManager.cpp",
        r"src\debug\DebugManager.cpp", r"src\graphics\models\Cube.cpp", r"src\graphics\models\Floor.cpp", r"src\graphics\Camera.cpp",
        r"third_party\glad\src\gl.c", r"third_party\imgui\imgui.cpp", r"third_party\imgui\imgui_draw.cpp",
        r"third_party\imgui\imgui_demo.cpp", r"third_party\imgui\imgui_tables.cpp", r"third_party\imgui\imgui_widgets.cpp",
    r"third_party\imgui\backends\imgui_impl_glfw.cpp", r"third_party\imgui\backends\imgui_impl_opengl3.cpp"
    ]

    flags = [
        f'-o "bin\LabProg.exe"',
        f'-I"third_party\imgui"',
        f'-I"third_party\imgui\backends"',
        f'-I"third_party\glad\include"', 
        f'-I"third_party\glfw\include"',
        f'-I"third_party\glm"',
        f'"{glfw_third_party_path}"',
        "-lopengl32 -lgdi32 -luser32 -lkernel32",
        "-std=c++17", "-static-third_partygcc -static-third_partystdc++"
    ]
